<template>
  <el-dialog :visible.sync="dialog" :title="isAdd ? '新增权限' : '编辑权限'" append-to-body width="500px" @close="cancel">
    <el-form ref="form" :model="form" :rules="rules" size="small" label-width="80px">
      ...
      <el-form-item label="权限定义" prop="permissionState">
        <url-select
          ref="permissionState"
          v-model="form.permissionState"
          :auto-load="true"
          :loaded-event="true"
          url="/api/permissions/state"
          id-name="id"
          label-name="label"
          @loaded="permissionStateLoaded"/>
      </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      form: { name: '', alias: '', permissionState: 'OTHER', systemOrderType: 'OTHER', pid: 0 },
      rules: {
        name: [
  ...
  methods: {
    ...
    permissionStateLoaded(sel) {
      console.log('message1', sel)
      sel.selectedValue(this.form.permissionState)
    },
    systemOrderTypeLoaded(sel) {
      sel.selectedValue(this.form.systemOrderType)
    },
    doSubmit() {
