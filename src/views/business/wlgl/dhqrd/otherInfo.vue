...
export default {
  ...
  data() {
    return {
      ...
      wlRules: {
        ...
        amount: [{ required: true, message: '数量必填' }]
      },
      // 其它按钮
      extraBtns: [
        {
  ...
  watch: {
    tableData: {
      handler(cVal, oVal) {
        cVal.forEach(item => {
          item.money = this.floatMul(item.amount, item.price)
          item.weight = this.floatMul(item.amount, item.netWeight)
        })
      },
      deep: true
    },
