...
export default {
  ...
  mixins: [initPostData, showPromptMsg, request],
  data() {
    return {
      activeName: 'unReview',
      form: {},
  ...
  methods: {
    beforeInit() {
      this.url = '/api/wuliu/queryDcyCancelInfo'
      ...
      this.$router.push({
        ...
        query: {
          data: JSON.stringify({
            ...
            wlCharge: row.wlCharge,
            operate: 7 // 1=竞价 2=新增竞价 3=编辑竞价 4=参与竞价与参与记录查看 5=重新竞价申请查看 6=编辑竞价申请查看 7=取消业务查看
          })
        }
      })
    },
    // 取消申请
    doCancel(row) {
      this.$confirm(`您正在尝试操作取消单据${row.orderNo}的业务取消申请，确定取消吗？`, '提示', {
